(function ( $ ) {
    ...
    function AvatarGenerate()
    {
        ...
        this.loadToClothes = function( type )
        {
            ...
            jQuery.each( load.sets, function( index, value )
            {
                if( value.gender == _this.gender && value.selectable == 1 || value.gender == "U" && value.selectable == 1 )
                {
                    ...
                    var asset  = $('<a href="#" class="page-content-choose-style-wrapper-item-content-look" data-clothing="' + index + '"><span class="page-content-choose-style-wrapper-item-content-look-figure" style="background-image: url(/templates/mezz/assets/images/client/figure/' + type + '-' + index + '.png);"></span></a>');
        ...
        {
            var URL = "https://www.habbo.com/habbo-imaging/avatarimage?head_direction=4&direction=4&size=l&figure=" + this.buildFigure() + "&gender=" + this.getGender();

            $( '#myHabbo' ).attr( 'src', URL );
            $( '#avatar_code' ).attr( 'value', this.buildFigure());
            $( '#avatar_gender' ).attr( 'value', this.getGender());
