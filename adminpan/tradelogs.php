					...
					<header class="panel-heading">
						...
						<div class="panel-body">
							<table class="table table-striped table-bordered table-condensed">
								...
								<tbody>
								<?php
									$getLogs = $dbh->prepare("SELECT room_trade_log.*, users.username FROM room_trade_log INNER JOIN users ON users.id = room_trade_log.id ORDER BY id DESC LIMIT 100");
									$getLogs->execute();
									while($log = $getLogs->fetch())
									{
										...
										if(!empty($log['2items'])){
											...
											foreach(array_diff( explode(";", $log['2items']), array('')) as $item){
												$itemLookup = $dbh->prepare("SELECT room_trade_log_items.base_item, catalog_items.catalog_name FROM room_trade_log_items INNER JOIN catalog_items ON catalog_items.item_id = room_trade_log_items.base_item WHERE items.id = ?");
												$itemLookup->execute(array($item));
												$item = $itemLookup->fetch(PDO::FETCH_ASSOC);
												$id = $item['base_item'];
												if(!$item){
