					...
					<header class="panel-heading">
						...
						<div class="panel-body">
							<table class="table table-striped table-bordered table-condensed">
								...
								<tbody>
								<?php
									$getArticles = $dbh->prepare("SELECT * FROM chatlogs_private ORDER BY id DESC  LIMIT 1000");
									$getArticles->execute();
									while($news = $getArticles->fetch())
									{
										echo'<tr>
										<td>'.$news["id"].'</td>
										<td style="width: 13%;">'.$news["user_from_id"].'</td>
										<td style="width: 7%;">'.$news["user_to_id"].'</td>
